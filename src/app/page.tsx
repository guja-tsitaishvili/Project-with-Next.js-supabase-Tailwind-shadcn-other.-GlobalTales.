...
export default function WelcomePage() {
  return (
    <main className="min-h-dvh grid grid-rows-[auto,1fr] bg-gradient-to-b from-background to-muted">
      ...
      <header className="sticky top-0 z-10 bg-background/80  backdrop-blur  supports-[backdrop-filter]:bg-background/70  border-b border-border">
        <div className="mx-auto max-w-screen-xl px-4 ">
          ...
          <h1 className="absolute left-1/2 -translate-x-1/2 select-none tracking-tight font-semibold leading-none text-2xl sm:text-3xl md:text-4xl">
            <span className="font-extrabold [letter-spacing:-0.02em]">Global</span>
            <span className="ml-1 font-extrabold italic [letter-spacing:-0.02em]">Tales</span>
          </h1>
           <a
        href="/login-register"
        className="ml-auto text-sm  px-3 py-1 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 "
      >Login</a>
        </div>
                </div>
      ...
      {/* Content area */}
      <section className="mx-auto w-full max-w-screen-xl grid grid-cols-1 lg:grid-cols-2 gap-6 px-4 py-8 sm:py-10 md:py-12">
       

     {/* --- HERO (put this inside the section where you left the comment) --- */}
<div className="relative overflow-visible rounded-xl ring-1 ring-border bg-card text-card-foreground">
  {/* Background image */}
  <div
...
>
  ...
  <div
    className="
      ...
      max-w-[96vw]                       /* phones: keep safe */
      sm:max-w-none sm:w-[40rem]         /* tablets */
      xl:w-[52rem]                       /* desktops */
      2xl:w-[56rem]                      /* ultrawide */
      2xl:[&>p]:line-clamp-2             /* make the headline 2 lines on 2xl */
      2xl:[&>p]:leading-tight
      2xl:[&>p]:text-balance
    "
  >
    <RotatingTexts onImage size="hero" align="left" />
        ...
        <aside className="hidden lg:block">
          <div className="h-full rounded-1xl border-l border-black/10 dark:border-white/10 bg-white/60 dark:bg-slate-950/40 p-6">
